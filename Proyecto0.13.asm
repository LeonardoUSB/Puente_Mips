###############################################
# CI-3815		                      #
# Proyecto 1: Calendario escritorio LCD       #
# Alejandro De Vincenzo  19-10109  	      #
# Leonardo Dolande       19-10181      	      #
###############################################
...
# Mensaje de prueba, ya no lo usamos.
...
# Nota: El código de la pila está hacia el final.
...
EneDes:		beqz $t2 ComienzoCalendario
		li $v0 4            			# Loop que Inprime la cantidad de espacios necesaria.
		la $a0 espacio        			# dependeiendo del desfase.
 		syscall
...
unidades:
	...
	beq $t7 7 salto2    			# Si el contador de dï¿½as por semana es igual a 7,
						# ir a "salto" para generar un salto de linea.
...
decenas:
	...
	beq $t7 7 salto2    			# Si el contador de días por semana es igual a 7,
						# ir a "salto" para generar un salto de lï¿½nea.
...
Teclado:
	...
	# Imprimir el mensaje (Ya no lo usamos).
	#li $v0, 4 
	#la $a0, mensaje 
	#syscall
...
SetMin1:					
	# Vuelve la primera parte del minuto a la normalidad e incrementa la segunda parte.
	li $t2 0
	sb $t2 Minuto2
	lb $t2 Minuto1
	add $t2, $t2,1
...
LastDia:
	# Evalúa si Dia se encontraba en último día del mes anterior al cambio.
	# En caso de que sea así, Dia se adapta al último día del nuevo mes.
	beq $s5 1 LastEnero	
	beq $s5 2 LastFebrero
	beq $s5 3 LastMarzo
	beq $s5 4 Last30
	beq $s5 6 Last30
	beq $s5 9 Last30
